...
function Schema(s, opts) {
	var self = this;

	this.options = _.defaults({
		asyncMiddlewareTimeout: 10000
	}, opts);
...
Schema.prototype.trigger = function(timing, event, model, cb) {
	...
	function next(err) {
		...
		if (subscriber._isAsync) {
			var asyncDef = Q.defer();
			promises.push(asyncDef.promise);
