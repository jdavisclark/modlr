...
function checkDescriptor(descriptor) {
	...
	// allowed (enum)
	if(descriptor.hasOwnProperty("allowed") && supportsEnum.indexOf(descriptor.type) === -1) {
		throw new Error("Type '" + types.getTypeString(descriptor.type) + "' does not supoprt enums!");
	}
}
