...
module.exports = function(schema) {
	...
	/* methods */
	var methodKeys = Object.keys(schema.methods);

	methodKeys.forEach(function(key) {
		// can only stick functions on the methods object...
		if(typeof schema.methods[key] !== "function") {
			return;
		}

		// cant override anything in the prototype chain.
		if(key in f.prototype) {
			throw new Error("'"+key+"' can not be overridden.");
		} else f.prototype[key] = schema.methods[key];
	});
